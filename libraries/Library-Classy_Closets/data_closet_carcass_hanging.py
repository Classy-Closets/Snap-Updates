...
ASSEMBLY_DIR = path.join(path.dirname(__file__),"Closet Assemblies")
...
def add_opening_prompts(product):
    ...
    for i in range(1,product.opening_qty+1):
        ...
        product.add_prompt(name="Opening " + str(i) + " Height",
                        prompt_type='DISTANCE',
                        value=unit.millimeter(float(props.hanging_panel_height)),
                        tab_index=1)
        
def add_common_closet_prompts(product):
    props = props_closet.get_scene_props()
    
    product.add_prompt(name="Left End to Floor",prompt_type='CHECKBOX',value=False,tab_index=1)
    product.add_prompt(name="Right End to Floor",prompt_type='CHECKBOX',value=False,tab_index=1)
    product.add_prompt(name="Rotate Left Filler",prompt_type='CHECKBOX',value=False,tab_index=1)
    product.add_prompt(name="Rotate Right Filler",prompt_type='CHECKBOX',value=False,tab_index=1)
    ...
    product.add_prompt(name="Left Filler Amount",prompt_type='DISTANCE',value=unit.inch(0),tab_index=1)
    product.add_prompt(name="Right Filler Amount",prompt_type='DISTANCE',value=unit.inch(0),tab_index=1)
    product.add_prompt(name="Filler Width",prompt_type='DISTANCE',value=unit.inch(4),tab_index=1)
    ...
    Rotate_Left_Filler = product.get_var('Rotate Left Filler')
    Rotate_Right_Filler = product.get_var('Rotate Right Filler')
    ...
    Left_Filler_Amount = product.get_var('Left Filler Amount')
    Right_Filler_Amount = product.get_var('Right Filler Amount')
    
    product.prompt("Left Filler Amount", 'IF(Rotate_Left_Filler,Panel_Thickness,Left_Side_Wall_Filler)',[Rotate_Left_Filler,Panel_Thickness,Left_Side_Wall_Filler])
    product.prompt("Right Filler Amount", 'IF(Rotate_Right_Filler,Panel_Thickness,Right_Side_Wall_Filler)',[Rotate_Right_Filler,Panel_Thickness,Right_Side_Wall_Filler])
    ...
    product.calculator_deduction("Left_Side_Thickness+Right_Side_Thickness+Panel_Thickness*(" + str(product.opening_qty) +"-1)+Left_Filler_Amount+Right_Filler_Amount",
                                 [Left_Side_Thickness,Right_Side_Thickness,Panel_Thickness,Left_Filler_Amount,Right_Filler_Amount])    
...
def add_sides(product):
    ...
    Left_End_Condition = product.get_var('Left End Condition')
    Right_End_Condition = product.get_var('Right End Condition')
    Left_End_to_Floor = product.get_var('Left End to Floor')
    Right_End_to_Floor = product.get_var('Right End to Floor')
    Rotate_Left_Filler = product.get_var('Rotate Left Filler')
    Rotate_Right_Filler = product.get_var('Rotate Right Filler')
    Filler_Width = product.get_var('Filler Width')
    Left_Filler_Amount = product.get_var('Left Filler Amount')
    Right_Filler_Amount = product.get_var('Right Filler Amount')
    
    left_filler = common_parts.add_filler(product)
    left_filler.x_dim('Height_1',[Height_1])
    left_filler.y_dim('IF(Rotate_Left_Filler,Filler_Width,-Left_Side_Wall_Filler)',[Rotate_Left_Filler,Filler_Width,Left_Side_Wall_Filler])
    left_filler.z_dim('-Left_Side_Thickness',[Left_Side_Thickness])
    left_filler.x_loc('Left_Side_Wall_Filler',[Left_Side_Wall_Filler])
    left_filler.y_loc('Product_Depth-IF(Add_Backing,Back_Thickness,0)',[Product_Depth,Add_Backing,Back_Thickness])
    left_filler.z_loc('IF(Left_End_to_Floor,0,Product_Height-Height_1)',[Left_End_to_Floor,Product_Height,Height_1])
    left_filler.x_rot(value = 0)
    left_filler.y_rot(value = -90)
    left_filler.z_rot('IF(Rotate_Left_Filler,0,-90)',[Rotate_Left_Filler])
    left_filler.prompt('Hide','IF(Rotate_Left_Filler,False,IF(Left_Side_Wall_Filler==0,True,False))',[Rotate_Left_Filler,Left_Side_Wall_Filler])
    
    left_side = common_parts.add_panel(product)
    left_side.x_dim('IF(Left_End_to_Floor,Product_Height,Height_1)',[Left_End_to_Floor,Product_Height,Height_1])
    left_side.y_dim('Product_Depth',[Product_Depth])
    left_side.z_dim('-Left_Side_Thickness',[Left_Side_Thickness])
    left_side.x_loc('Left_Filler_Amount',[Left_Filler_Amount])
    left_side.y_loc('IF(Add_Backing,-Back_Thickness,0)',[Add_Backing,Back_Thickness])
    left_side.z_loc('IF(Left_End_to_Floor,0,Product_Height-Height_1)',[Left_End_to_Floor,Product_Height,Height_1])
    ...
    right_filler.y_dim('IF(Rotate_Right_Filler,Filler_Width,Right_Side_Wall_Filler)',[Rotate_Right_Filler,Filler_Width,Right_Side_Wall_Filler])
    ...
    right_filler.y_loc('Product_Depth-IF(Add_Backing,Back_Thickness,0)',[Product_Depth,Add_Backing,Back_Thickness])
    right_filler.z_loc('IF(Right_End_to_Floor,0,Product_Height-Last_Height)',[Right_End_to_Floor,Product_Height,Last_Height])
    right_filler.x_rot(value = 0)
    right_filler.y_rot(value = -90)
    right_filler.z_rot('IF(Rotate_Right_Filler,0,-90)',[Rotate_Right_Filler])
    right_filler.prompt('Hide','IF(Rotate_Right_Filler,False,IF(Right_Side_Wall_Filler==0,True,False))',[Right_Side_Wall_Filler,Rotate_Right_Filler])

    right_side = common_parts.add_panel(product)
    right_side.x_dim('IF(Right_End_to_Floor,Product_Height,Last_Height)',[Right_End_to_Floor,Product_Height,Last_Height])
    right_side.y_dim('Product_Depth',[Product_Depth])
    right_side.z_dim('Right_Side_Thickness',[Right_Side_Thickness])
    right_side.x_loc('Product_Width-Right_Filler_Amount',[Product_Width,Right_Filler_Amount])
    right_side.y_loc('IF(Add_Backing,-Back_Thickness,0)',[Add_Backing,Back_Thickness])
    right_side.z_loc('IF(Right_End_to_Floor,0,Product_Height-Last_Height)',[Right_End_to_Floor,Product_Height,Last_Height])
...
def add_panel(product,index,previous_panel):
    ...
    if previous_panel:
        ...
        Left_Side_Thickness = product.get_var('Left Side Thickness')
        panel.x_loc('Left_Side_Thickness+Width+Left_Filler_Amount+Panel_Thickness',
                    [Left_Side_Thickness,Width,Left_Filler_Amount,Panel_Thickness])
...
def add_full_top(product):
    ...
    top.z_loc('Product_Height',[Product_Height])
    top.x_rot(value = 0)
    top.y_rot(value = 0)
    top.z_rot(value = 0)
    top.x_dim('Product_Width',[Product_Width])
    top.y_dim('Product_Depth',[Product_Depth])
...
def add_shelf(product,i,panel,is_top=False):
    ...
    if panel:
        ...
        shelf.x_loc('Left_Filler_Amount+X_Loc',[Left_Filler_Amount,X_Loc])
    
    shelf.y_loc('IF(Add_Backing,-Back_Thickness,0)',[Add_Backing,Back_Thickness])
    shelf.z_loc('Product_Height-Height',[Toe_Kick_Height,Product_Height,Height])
...
def add_toe_kick(product,i,panel):
    ...
    kick = common_parts.add_toe_kick(product)
    kick.y_dim('-Toe_Kick_Height',[Toe_Kick_Height,Shelf_Thickness])
    kick.z_dim(value = unit.inch(.75))
    kick.y_loc('Product_Depth+Toe_Kick_Setback-IF(Add_Backing,Back_Thickness,0)',[Product_Depth,Toe_Kick_Setback,Add_Backing,Back_Thickness])
    kick.z_loc(value = 0)
    kick.x_rot(value = -90)
    kick.y_rot(value = 0)
    kick.z_rot(value = 0)
    kick.prompt("Hide",'IF(Product_Height==(Height-Toe_Kick_Height),False,True)',[Product_Height,Height,Toe_Kick_Height])
...
def add_panel_drilling(part,product,index):
    ...
    token.face_bore_dia = machining_props.system_hole_dia
...
class Closet_Carcass(fd_types.Assembly):
    ...
    category_name = "Floor Closet Openings"
    property_id = props_closet.LIBRARY_NAME_SPACE + ".full_top_openings"
...
class PROMPTS_Full_Top_Opening_Starter(bpy.types.Operator):
    
    #TODO: Turn off equal width property if all but one equal options are checked.
    
    bl_idname = props_closet.LIBRARY_NAME_SPACE + ".full_top_openings"
            ...
            if opening_height_prompt:
                ...
                for index, height in enumerate(common_lists.PANEL_HEIGHTS):
                    if not opening_height >= int(height[0]):
                        if i == 1:
                            self.Opening_1_Height = common_lists.PANEL_HEIGHTS[index - 1][0]
                        if i == 2:
                            self.Opening_2_Height = common_lists.PANEL_HEIGHTS[index - 1][0]
                        if i == 3:
                            self.Opening_3_Height = common_lists.PANEL_HEIGHTS[index - 1][0]
                        if i == 4:
                            self.Opening_4_Height = common_lists.PANEL_HEIGHTS[index - 1][0]
                        if i == 5:
                            self.Opening_5_Height = common_lists.PANEL_HEIGHTS[index - 1][0]
                        if i == 6:
                            self.Opening_6_Height = common_lists.PANEL_HEIGHTS[index - 1][0]
                        if i == 7:
                            self.Opening_7_Height = common_lists.PANEL_HEIGHTS[index - 1][0]
                        if i == 8:
                            self.Opening_8_Height = common_lists.PANEL_HEIGHTS[index - 1][0]                                                                                                                                                                                                                 
                        break
    ...
    def draw_common_prompts(self,layout):
        rotate_left_filler = self.product.get_prompt("Rotate Left Filler")
        rotate_right_filler = self.product.get_prompt("Rotate Right Filler")
        extend_left = self.product.get_prompt("Left End to Floor")
        extend_right = self.product.get_prompt("Right End to Floor")        
                
        box = layout.box()
        col = box.column(align=True)
        row = col.row()
        row.label("Filler:")
        row.prop(rotate_left_filler,rotate_left_filler.prompt_type,"Left")
        row.prop(rotate_right_filler,rotate_right_filler.prompt_type,"Right")      
        row = col.row()
        row.label("Extend Side:")
        row.prop(extend_left,extend_left.prompt_type,"Left")
        row.prop(extend_right,extend_right.prompt_type,"Right")   
...
bpy.utils.register_class(PROMPTS_Full_Top_Opening_Starter)
