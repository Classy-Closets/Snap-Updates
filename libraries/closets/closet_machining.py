...
MACHINING_PROPERTY_NAMESPACE = "closet_machining"
...
class SN_UL_Mac_Scenes(bpy.types.UIList):
    ...
    def filter_items(self, context, data, propname):
        ...
        for i, scene in enumerate(scenes):
            if scene.snap.plan_view_scene or scene.snap.elevation_scene:
...
class OPERATOR_Prepare_Closet_For_Export(bpy.types.Operator):
    ...
    def add_hardware(self,name,assembly):
        obj = sn_utils.create_single_vertex(name)
    ...
    def get_product_from_part(self,part):
        product_bp = sn_utils.get_bp(part.obj_bp,'PRODUCT')
        return sn_types.Assembly(product_bp)
    
    def get_hinge_qty(self,length):
        if length > sn_unit.inch(48): 
            return 3
        else:
            return 2    
    ...
    def add_associative_hinges(self,assembly):
        ...
        if door_swing:
            if door_swing.get_value() in {"Top","Bottom"}:
        ...
        hole_qty = math.floor(sn_unit.meter_to_millimeter(spacing) / 32)
        spacing_32mm = hole_qty * sn_unit.millimeter(32)
        ...
        for i in range(0,hinge_qty):            
            if door_swing.get_value() == 0 or door_swing.get_value() == 1:  # Left or Right
                ...
                if macp.use_associative_hardware_for_hinges:
                    ...
                    hinge.snap.associative_rotation = 270 if door_swing.get_value() == 'Left' else 90
                ...
                if macp.use_associative_hardware_for_hinge_plates:
                    ...
                    plate.snap.associative_rotation = 0
            ...
            if door_swing.get_value() == 2:  # Top
                if macp.use_associative_hardware_for_hinges:
                    ...
                    hinge.location.x = macp.hinge_location_from_top_and_bottom + spacing_32mm*i
                    hinge.snap.associative_rotation = 180
    ...
    #def add_door_pull_drilling(self,assembly):
        ...
        #             if door_swing.get_value() == 'Bottom':
        ...
        #         elif door_type.get_value() == 'Upper':
    ...
    def add_associative_pull(self,assembly):
        ...
        pull.location.y = 0
        pull.location.x = 0
        pull.snap.associative_rotation = 0
    ...
    def add_blind_corner_panel_drilling(self,assembly):
        ...
        if left.get_value():
            ...
            token.end_dim_in_x =  -sn_unit.inch(0.455) + macp.dim_to_system_top_hole + macp.dim_between_holes
    ...
    def add_corner_back_drilling(self,assembly):
        ...
        part_length = math.fabs(assembly.obj_x.location.x)
        part_width = math.fabs(assembly.obj_y.location.y)
        dim_to_front = sn_unit.inch(3)
        dim_to_rear = sn_unit.inch(3)

        obj, token = assembly.add_machine_token('System Holes Right Bottom Rear','BORE','5')
        token.dim_in_x = part_length - sn_unit.millimeter(22.475)
        token.dim_in_y = part_width - dim_to_rear
        token.dim_in_z = macp.system_hole_bore_depth
        token.end_dim_in_x =  sn_unit.millimeter(22.475)
    ...
    def add_panel_drilling(self,assembly):
        ...
        if macp.panel_drilling_type == '32MM':            
            ...
            sdbl = assembly.get_prompt('Stop Drilling Bottom Left').get_value()
            ...
            corbeled_partition = False
            corbel_pard_ppt = assembly.get_prompt("Corbel Partition")
            corbel_height_ppt = assembly.get_prompt("Corbel Height")
            if corbel_pard_ppt and corbel_height_ppt:
                if corbel_pard_ppt.get_value():
            ...
            if single_sided:
                ...
                if left_depth.get_value() > 0 or sdbl > 0 or sdbr > 0:
                    obj, token = assembly.add_machine_token('System Holes Left Bottom Front','BORE','5')
                    token.dim_in_y = left_depth.get_value() - dim_to_front
                    token.end_dim_in_x = macp.dim_to_system_bottom_hole if sdbl == 0 else sdbl
                    token.end_dim_in_y = left_depth.get_value() - dim_to_front
                    tokens.append(token)
            ...
            # if right_depth.get_value() > 0 and sdtr > 0:
            #     obj, token = assembly.add_machine_token('System Holes Right Top Front','BORE','5')
            #     token.dim_in_y = dim_to_front + (part_width - right_depth.get_value())
            #     token.end_dim_in_x = part_length - sdtr
            #     token.end_dim_in_y = dim_to_front + (part_width - right_depth.get_value())
            #     tokens.append(token)
            ...
            #     if left_depth.get_value() != right_depth.get_value() and left_depth.get_value() > 0:
            #         obj, token = assembly.add_machine_token('System Holes Left Top Front','BORE','6')
            #         token.dim_in_y = dim_to_front + (part_width - left_depth.get_value())
            #         token.end_dim_in_x = part_length - sdtl
            #         token.end_dim_in_y = dim_to_front + (part_width - left_depth.get_value())
            #         tokens.append(token)
    ...
    def add_panel_drilling_for_middle_hanging_rods(self, hanging_rod):
        ...
        if insert_bp:
            ...
            depth = insert.obj_y.location.y
            extra_deep_pard = insert.get_prompt("Extra Deep Pard")

            setback = None

            rod_setback = None
            bottom_shelf_setback = None
            if extra_deep_pard and (depth >= extra_deep_pard.get_value()):
                rod_setback = insert.get_prompt("Add Deep Rod Setback")
                bottom_shelf_setback = insert.get_prompt("Add Bottom Deep Shelf Setback")
            else:
                rod_setback = insert.get_prompt("Add Rod Setback")
                bottom_shelf_setback = insert.get_prompt("Add Bottom Shelf Setback")
            top_shelf_setback = insert.get_prompt("Add Top Shelf Setback")
            middle_shelf_setback = insert.get_prompt("Add Middle Shelf Setback")

            setback_prompts = [rod_setback, top_shelf_setback, middle_shelf_setback, bottom_shelf_setback]

            if all(setback_prompts):
                setback = rod_setback
                if top_shelf_setback.get_value() > setback.get_value():
                    setback = top_shelf_setback
                if middle_shelf_setback.get_value() > setback.get_value():
                    setback = middle_shelf_setback
    ...
    def add_panel_routing(self,assembly):
        ...
        if macp.include_routing_on_panels:
            ...
            if First_Rear_Notch_Height and First_Rear_Notch_Depth and Second_Rear_Notch_Height and Second_Rear_Notch_Depth:
                ...
                if First_Rear_Notch_Height.get_value() > 0 and loc_z < sn_unit.inch(1):
                    obj, first_notch = assembly.add_machine_token('First Rear Notch' ,'CORNERNOTCH','5','3')
                    first_notch.dim_in_x = First_Rear_Notch_Height.get_value()
                    first_notch.dim_in_y = First_Rear_Notch_Depth.get_value()
                    first_notch.dim_in_z = panel_thickness
                    first_notch.lead_in = macp.router_lead
                    first_notch.tool_number = macp.tool_number
                
                if Second_Rear_Notch_Height.get_value() > 0 and loc_z < sn_unit.inch(1):
                    obj, second_notch = assembly.add_machine_token('Second Rear Notch' ,'CORNERNOTCH','5','3')
                    second_notch.dim_in_x = Second_Rear_Notch_Height.get_value()
                    second_notch.dim_in_y = First_Rear_Notch_Depth.get_value() + Second_Rear_Notch_Depth.get_value()
            ...
            if Front_Angle_Depth and Front_Angle_Height and Rear_Angle_Height and Rear_Angle_Depth:
                ...
                if Front_Angle_Height.get_value() > 0:
                    obj, front_chamfer = assembly.add_machine_token('Front Chamfer' ,'CHAMFER','5','7')
                    front_chamfer.dim_in_x = Front_Angle_Height.get_value()
                    front_chamfer.dim_in_y = Front_Angle_Depth.get_value()
                    front_chamfer.dim_in_z = panel_thickness
                    front_chamfer.lead_in = macp.router_lead
                    front_chamfer.tool_number = macp.tool_number
                    
                if Rear_Angle_Height.get_value() > 0:
                    obj, rear_chamfer = assembly.add_machine_token('Rear Chamfer' ,'CHAMFER','5','5')
                    rear_chamfer.dim_in_x = Rear_Angle_Height.get_value()
                    rear_chamfer.dim_in_y = Rear_Angle_Depth.get_value()
    ...
    def add_shelf_and_rod_cleat_holes(self,assembly):
        ...
        if product_bp:
            ...
            rod_placement = product.get_prompt("Hanging Rod Placement").get_value()
            rod_vertical_placement = product.get_prompt("Hanging Vertical Placement").get_value()

            if add_rod_holes and add_rod_holes.get_value():
                
                panel_length = math.fabs(assembly.obj_x.location.x)
                panel_width = math.fabs(assembly.obj_y.location.y)
                
                face = "6" if "Left" in assembly.obj_bp.snap.name_object else "5"
                
                obj, token = assembly.add_machine_token('Shelf and Rod Holes','BORE',face)
                token.dim_in_x = rod_placement
                token.dim_in_y = rod_vertical_placement - (sn_unit.millimeter(32)/2)
                token.dim_in_z = sn_unit.millimeter(15)
                token.face_bore_dia = macp.system_hole_dia
                token.end_dim_in_x = rod_placement
                token.end_dim_in_y = rod_vertical_placement + (sn_unit.millimeter(32)/2)
                token.distance_between_holes = sn_unit.millimeter(32)
                token.associative_dia = 0
                token.associative_depth = 0
    ...
    def add_shelf_and_rod_fe_machining(self,assembly):
        ...
        if product_bp:
            ...
            rod_placement = product.get_prompt("Hanging Rod Placement").get_value()
            rod_vertical_placement = product.get_prompt("Hanging Vertical Placement").get_value()
            left_depth = assembly.get_prompt("Left Depth").get_value()
            ...
            if add_rod_holes and add_rod_holes.get_value():
                ...
                obj, token = assembly.add_machine_token('Shelf and Rod Holes','BORE',face)
                token.dim_in_x = rod_vertical_placement  - (sn_unit.millimeter(32)/2)
                token.dim_in_y = panel_length - rod_placement
                token.dim_in_z = sn_unit.millimeter(15)
                token.face_bore_dia = macp.system_hole_dia
                token.end_dim_in_x = rod_vertical_placement + (sn_unit.millimeter(32)/2)
    ...
    def add_panel_hanging_machining(self,assembly):
        ...
        if macp.include_routing_for_hanging_notch:
            ...
            if is_right_end_panel.get_value():
                face = '6'
            else:
                face = '5'
            ...
            if is_right_end_panel.get_value() or is_left_end_panel.get_value():
        ...
        if macp.include_drilling_for_hanging_hardware:
            ...
            if place_hanging_hardware_on_right.get_value():
                face = '5'
            else:
                face = '6'
    ...
    def add_drawer_wood_box_machining(self,assembly):
        ...
        if macp.wood_drawer_box_machining == 'DOWELS':
            ...
            token.dim_to_first_const_hole = sn_unit.inch(1.25)
            token.dim_to_last_const_hole = width - sn_unit.inch(1.25)
    ...
    def add_drawer_bottom_dado(self,assembly):
        ...
        if macp.route_dado_for_drawer_bottom:
            ...
            for token in tokens:
                ...
                token[1].beginning_depth = sn_unit.inch(.25) + sn_unit.inch(.01)
                token[1].double_pass = thickness + sn_unit.inch(.1)
                token[1].panel_penetration = sn_unit.inch(-.25)     
    ...
    def add_slanted_shelf_machining(self,assembly):
        ...
        if macp.slanted_shoe_shelf_machining_type == 'ONLYDRILLREARCAMS':
            ...
            vector = token.vector_locations.add()
            vector.x_loc = length + sn_unit.inch(.01)
            vector.y_loc = width - macp.slanted_shoe_shelf_machining_distance_from_rear
            vector.z_loc = sn_unit.inch(.125)
    
            vector = token.vector_locations.add()
            vector.x_loc = length - sn_unit.inch(.5)
            vector.y_loc = width - macp.slanted_shoe_shelf_machining_distance_from_rear
            vector.z_loc = sn_unit.inch(.125)           
            ...
            vector = token.vector_locations.add()
            vector.x_loc = -sn_unit.inch(.01)
            vector.y_loc = width - macp.slanted_shoe_shelf_machining_distance_from_rear
            vector.z_loc = sn_unit.inch(.125)            
    
            vector = token.vector_locations.add()
            vector.x_loc = sn_unit.inch(.5)
            vector.y_loc = width - macp.slanted_shoe_shelf_machining_distance_from_rear
            vector.z_loc = sn_unit.inch(.125)                     
    ...
    def add_l_shelf_routing(self,assembly):
        ...
        second_notch.dim_in_x = part_length - Left_Depth.get_value()
        second_notch.dim_in_y = part_width - Right_Depth.get_value()
        second_notch.dim_in_z = panel_thickness
        second_notch.lead_in = macp.router_lead
        second_notch.tool_number = macp.tool_number
    ...
    def add_slide_token(self,assembly):
        ...
        if macp.use_associative_hardware_for_slides:
            ...
            if options.slide_name == 'Accuride Slides':
                ...
                token.dim_to_first_hole = sn_unit.inch(1.5817) if depth > sn_unit.inch(1.5817) else 0
                token.dim_to_second_hole = sn_unit.inch(6.6211) if depth > sn_unit.inch(6.6211) else 0
                token.dim_to_third_hole = sn_unit.inch(10.4006) if depth > sn_unit.inch(10.4006) else 0
                token.dim_to_fourth_hole = sn_unit.inch(15.44) if depth > sn_unit.inch(15.44) else 0
                token.dim_to_fifth_hole = sn_unit.inch(17.9596) if depth > sn_unit.inch(17.9596) else 0
    ...
    def set_manufacturing_material(self,assembly):
        ...
        for child in assembly.obj_bp.children:
            if child.snap.type_mesh == 'CUTPART':
                ...
                edgepart_name = child.snap.edgepart_name
                
                edge_part_pointers = bpy.context.scene.snap.spec_groups[0].edgeparts
                cut_part_pointers = bpy.context.scene.snap.spec_groups[0].cutparts
                material_pointers = bpy.context.scene.snap.spec_groups[0].materials
                ...
                if cutpart_name in cut_part_pointers:
                    ...
                    material_thickness = str(round(sn_unit.meter_to_active_unit(pointer.thickness),4))
                    
                    child.snap.cutpart_material_name = material_thickness + " " + material_name
                ...
                if edgepart_name in edge_part_pointers:
                    ...
                    edgeband_thickness = str(round(sn_unit.meter_to_active_unit(pointer.thickness),4))
                    
                    child.snap.edgeband_material_name = edgeband_thickness + " " + material_name    
    ...
    def set_section_number(self,assembly):
        """
            ...
            This can only be set for parts that have a integer assigned for the sn_closets.opening_name property
        """
        ...
        if wall_bp:
            ...
            for product in products:
                if product == product_bp:
                    assembly.obj_bp.snap.comment_2 = str(int(round(float(product.sn_closets.opening_name),0)) + adjusted_opening_number)
                    break
                else:
                    adjusted_opening_number += int(round(float(product.sn_closets.opening_name),0))
    ...
    def get_adj_panels(self,insert_bp):
        product_bp = sn_utils.get_bp(insert_bp,'PRODUCT')
        panels = self.get_product_panels(product_bp,[])
        right_panel_bp = None
        left_panel_bp = None
        # Had to change how we get left and right panels as using location.x did not work for hanging rod and shelf assemblies that were above other assemblies
        ...
        right_panel = sn_types.Part(right_panel_bp)
    ...
    def update_panel_prompts_for_drawer_stack(self,drawer_stack):
        ...
        dist_from_top = drawer_stack.obj_z.location.z - drawer_stack_height.get_value()
                    
        stop_drilling_bottom_left.set_value(0)
        stop_drilling_bottom_right.set_value(0)
        stop_drilling_top_left.set_value(dist_from_top + sn_unit.millimeter(32))
        stop_drilling_top_right.set_value(dist_from_top + sn_unit.millimeter(32))
    ...
    def execute(self, context):
        ...
        for assembly in closet_utils.scene_parts(context):
            ...
            if assembly.obj_bp.get('IS_BP_MITERED_PARD'):
            ...
            if assembly.obj_bp.get('IS_CORNER_BACK'):
...
class PROPS_Machining_Defaults(bpy.types.PropertyGroup):
    ...
    #                                          default=sn_unit.millimeter(37),unit='LENGTH')
    ...
    dim_to_toe_kick_cam: FloatProperty(name="Dim To Toe Kick Cam",
                                            ...
                                            default=sn_unit.inch(1.25),unit='LENGTH')    
    ...
    hanging_notch_routing_depth_on_end_panels: FloatProperty(name="Hanging Notch Routing Depth on End Panels",
                                            ...
                                            default=sn_unit.inch(.5625),unit='LENGTH')       
    ...
    hanging_hardware_second_drilling_hole_from_back_of_panel: FloatProperty(name="Hanging Hardware Second Drilling Hole from Back of Panel",
                                            ...
                                            default=sn_unit.inch(2.5),unit='LENGTH')      
    ...
    cam_backset: FloatProperty(name="Cam Backset",
                              ...
                              default=sn_unit.millimeter(9.5),unit='LENGTH')       
    ...
    router_lead: FloatProperty(name="Router Lead",default=sn_unit.inch(.25),unit='LENGTH')
    ...
    hinge_cup_off_door_edge: FloatProperty(name="Hinge Cup Off Door Edge",default=sn_unit.inch(.9252),unit='LENGTH')
     
    hinge_plate_inset_from_door: FloatProperty(name="Hinge Plate Inset From Door",default=sn_unit.inch(1.4567),unit='LENGTH')

    hinge_location_from_top_and_bottom: FloatProperty(name="Hinge Location From Top and Bottom",default=sn_unit.inch(3),unit='LENGTH')
    ...
    pull_location_on_tall_doors: FloatProperty(name="Pull Location on Tall Doors",
                                                ...
                                                default=sn_unit.inch(40),unit='LENGTH')
    ...
    door_pull_length: FloatProperty(name="Door Pull Length",
                                     ...
                                     default=sn_unit.inch(3),unit='LENGTH')
    ...
    #SLIDE SETTINGS
    slide_face_bore_depth: FloatProperty(name="Slide Face Bore Depth",default=sn_unit.inch(.5511),unit='LENGTH')
    ...
    drawer_slide_dim_from_bottom: FloatProperty(name="Drawer Slide Dim From Bottom",default=sn_unit.inch(.5),unit='LENGTH')
    
    drawer_slide_gap: FloatProperty(name="Drawer Slide Gap",default=sn_unit.inch(.5),unit='LENGTH')
    ...
    def draw(self,layout):
        ...
        if self.machining_tabs == 'MAIN':
            ...
            # box = main_col.box()
            # col = box.column()
            # col.label(text="Drawer Box Machining:")        
            # row = col.row()
        ...
        if self.machining_tabs == 'HARDWARE':
            ...
            if self.use_associative_hardware_for_slides:
                row = col.row()
                row.label(text="Drawer Slide Gap:")
                row.prop(self,'drawer_slide_gap',text="")
                row = col.row()
                row.label(text="Dim from Bottom of Drawer:")
            ...
            hardware_box = main_col.box()
            col = hardware_box.column()
            col.label(text="Add Associative Hinge Hardware:")
            row = col.row()
            ...
            # hardware_box = main_col.box()
            # hardware_box.label(text="Closet Rod Cups:")
        ...
        #     plan_box = main_col.box()
        #     plan_box.label(text="Elevation View:")
        #     plan_box.prop(self,'use_associative_hardware_for_slides',text="Add Section Widths")
...
class SN_PT_Closet_Machining_Setup(bpy.types.Panel):
