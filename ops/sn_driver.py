...
class PromptCollection(PropertyGroup):
    add: BoolProperty(name="Add")
    prompt_type: StringProperty(name="Type")


class SN_DRV_OT_get_vars_from_object(Operator):
    bl_idname = "sn_driver.get_vars_from_object"
    ...
    prompts: CollectionProperty(name='Collection Prompts',type=PromptCollection)
    ...
    def get_prompts(self):
        ...
        if self.assembly:
            for prompt in self.assembly.obj_prompts.snap.prompts:
                prompt_copy = self.prompts.add()
                prompt_copy.name = prompt.name
                prompt_copy.prompt_type = prompt.prompt_type
        else:
            for prompt in self.obj.snap.prompts:
                prompt_copy = self.prompts.add()
                prompt_copy.name = prompt.name
                prompt_copy.prompt_type = prompt.prompt_type

    def execute(self, context):
        drivers = sn_utils.get_drivers(self.obj)
        for DR in drivers:
                ...
                for prompt in self.prompts:
                    if prompt.add:
                        ...
                        if prompt.prompt_type == 'FLOAT':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].float_value'
                        if prompt.prompt_type == 'DISTANCE':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].distance_value'
                        if prompt.prompt_type == 'ANGLE':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].angle_value'
                        if prompt.prompt_type == 'QUANTITY':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].quantity_value'
                        if prompt.prompt_type == 'PERCENTAGE':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].percentage_value'
                        if prompt.prompt_type == 'CHECKBOX':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].checkbox_value'
                        if prompt.prompt_type == 'COMBOBOX':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].combobox_index'
                        if prompt.prompt_type == 'TEXT':
                            var.targets[0].data_path = 'snap.prompts["' + prompt.name + '"].text_value'
    ...
    def invoke(self,context,event):
        ...
        obj_bp = sn_utils.get_assembly_bp(var_obj)

        if obj_bp:
            self.assembly = sn_types.Assembly(obj_bp=obj_bp)
...
class SN_DRV_OT_remove_variable(Operator):
    bl_idname = "sn_driver.remove_variable"
...
class SN_DRV_OT_add_driver(Operator):
    bl_idname = "sn_driver.add_driver"
    bl_label = "Add Driver"
    ...
    def execute(self, context):
        ...
        assembly = sn_types.Assembly(obj_bp=assembly_bp)
...
class SN_DRV_OT_remove_driver(Operator):
    bl_idname = "sn_driver.remove_driver"
...
class SN_DRV_OT_add_calculator_driver(Operator):
    bl_idname = "sn_driver.add_calculator_driver"
    ...
    def execute(self, context):
        assembly_bp = sn_utils.get_assembly_bp(context.object)    
        assembly = sn_types.Assembly(obj_bp=assembly_bp)
        calculator = assembly.obj_prompts.snap.calculators[assembly.obj_prompts.snap.calculator_index]

        if calculator.distance_obj:
            calculator.distance_obj.driver_add('snap.calculator_distance')
            
        return {'FINISHED'}


class SN_DRV_OT_remove_calculator_driver(Operator):
    bl_idname = "sn_driver.remove_calculator_driver"
    ...
    def execute(self, context):
        ...
        if calculator.distance_obj:
            calculator.distance_obj.driver_remove('snap.calculator_distance')
...
classes = (
    PromptCollection,
    SN_DRV_OT_get_vars_from_object,
    SN_DRV_OT_remove_variable,
    SN_DRV_OT_add_driver,
    SN_DRV_OT_remove_driver,
    SN_DRV_OT_add_calculator_driver,
    SN_DRV_OT_remove_calculator_driver,
)
