...
class OPERATOR_Assign_Materials(bpy.types.Operator):
    ...
    update_countertops = bpy.props.BoolProperty(
        name="Update Countertops", default=False)
    only_update_pointers = bpy.props.BoolProperty(
...
class OPERATOR_Unpack_Material_Images(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        search_directory = os.path.join(
            materials_dir, "Cabinet Materials", "Classy Closets")
...
class OPERATOR_Create_Material_Library(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        template_dir = os.path.join(
            materials_dir, "Cabinet Materials", "Classy Closets", "_template_material")
        template_file = os.path.join(materials_dir, "Cabinet Materials",
                                     "Classy Closets", "_template_material", "_template_material.blend")
        ...
        for image_name in os.listdir(tex_directory):
            ...
            script_file = open(script, 'w')
            script_file.write("import bpy\n")
            script_file.write("bpy.ops.image.open(filepath=r'" + tex_file_path +
                              "', files=[{'name':'" + image_name + "', 'name':'" + image_name + "'}], relative_path=False, show_multiview=False)\n")
            script_file.write(
                "bpy.data.materials['_template_material'].node_tree.nodes['Image Texture'].image = bpy.data.images['" + image_name + "']\n")
            script_file.write(
                "bpy.data.materials['_template_material'].name = '" + image_name.split(".")[0] + "'\n")
