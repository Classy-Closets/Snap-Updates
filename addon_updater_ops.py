...
class AddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if updater.invalid_updater:
            ...
            col = layout.column()
            col.label(text="Update to Version {}".format(str(updater.update_version)), icon="LOOP_FORWARDS")
    ...
    def draw(self, context):
        ...
        if self.error != "":
            ...
            if "just_restored" in saved and saved["just_restored"]:
                ...
                alert_row.operator(
                    "wm.quit_blender",
                    text="Restart SNaP to complete update.",
                    icon="BLANK1")
                updater.json_reset_restore()
            else:
                col = layout.column()
                col.label(
                    text="SNaP successfully installed", icon="FILE_TICK")
                alert_row = col.row()
                alert_row.alert = True
                alert_row.operator(
                    "wm.quit_blender",
