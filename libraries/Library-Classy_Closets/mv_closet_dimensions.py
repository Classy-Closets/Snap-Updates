...
class OPERATOR_Auto_Dimension(bpy.types.Operator):
    ...
    def add_panel_height_annotation(self,assembly):
        ...
        dim.set_label(str(opengl_dim.format_distance(assembly.obj_x.location.x)))
        ...
        if wall_bp:
            ...
            for product in products:
                if product == product_bp:
                    assembly.obj_bp.mv.comment_2 = str(int(assembly.obj_bp.mv.opening_name) + adjusted_opening_number)
                    break
                else:
    ...
    def add_fixed_shelf_and_rod_annotations(self,product):
        ...
        dist_r = math.fabs(product.obj_bp.location.x + product.obj_x.location.x - right_x)
        if dist_r > 0:
    ...
    def add_closet_annotations(self,closets):
        ...
        for closet_bp in closets:
            ...
            dist_r = math.fabs(closet.obj_bp.location.x + closet.obj_x.location.x - right_x)
            if dist_r > 0:
            ...
            for i in range(1,10):
                ...
                opening_width = closet.get_prompt("Opening " + str(i) + " Width")
                ...
                if opening_height and opening_width and panel_thickness:
                    ...
                    if scene_props.add_closet_height:
                        if closet.obj_z.location.z not in hanging_heights:
                            hanging_height = self.add_dimension(closet.obj_bp.parent)
                            hanging_height.start_x(value = unit.inch(-6) * (len(hanging_heights) + 1))
            ...
            if props.is_fixed_shelf_and_rod_product_bp:
                ...
                if right_fin_end and right_fin_end.value():
                    ...
                    right_fe.start_x(value = closet.obj_x.location.x + unit.inch(2))
                    right_fe.set_label("Triangle")                    
                
                if scene_props.add_section_widths:
                    width = self.add_dimension(closet.obj_bp)
                    width.start_x(value = panel_thickness.value())  
                    width.end_x(value = closet.obj_x.location.x - panel_thickness.value()*2)        
                    width.start_z(value = - dim_from_floor - unit.inch(4))               
...
#                 self.add_fixed_shelf_and_rod_annotations(assembly)
            ...
            if props.is_shelf_bp:
                ...
                if is_locked_shelf and is_locked_shelf.value() and scene_props.label_lock_shelves:
                    ...
                    shelf_label.start_z(value = -math.fabs(assembly.obj_z.location.z/2))                 
...
#                     shelf_label.set_label("ADJ")
            ...
            if props.is_door_bp and scene_props.label_door_height:
                ...
                dim_df_height.start_x(value = assembly.obj_x.location.x/2)
            ...
            if props.is_drawer_front_bp and scene_props.label_drawer_front_height:
                dim_df_height = self.add_dimension(assembly.obj_bp)
                dim_df_height.start_x(value = unit.inch(.5))
...
class PROPS_2D_Dimensions(bpy.types.PropertyGroup):
    ...
    def draw(self,layout):
        ...
        row = main_box.row()
        row.scale_y = 1.3
        row.operator(DIMENSION_PROPERTY_NAMESPACE + ".auto_dimension",icon='FILE_TICK')
...
exec("bpy.types.Scene." + DIMENSION_PROPERTY_NAMESPACE + "= PointerProperty(type = PROPS_2D_Dimensions)")
exec("bpy.types.Object." + DIMENSION_PROPERTY_NAMESPACE + "= PointerProperty(type = PROPS_Object_2D_Dimensions)")
...
def add_annotations_and_dimensions(scene=None):
    ...
    if props.auto_add_on_save:
        print('AUTOADDDIMS',"bpy.ops." + DIMENSION_PROPERTY_NAMESPACE + ".auto_dimension")
